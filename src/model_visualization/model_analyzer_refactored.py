...
# 初始化日志器
logger = get_logger(
    name=__name__, log_file="logs/model_analysis.log", global_level="INFO"
)
...
from src.model_visualization.data_models import MetricData, ModelInfoData
...
class ResultVisualizer:
    ...
    def sort_by_metric(
        ...
        return sorted(valid_items, key=lambda x: x.metrics[metric_key], reverse=reverse)
    ...
    def print_summary_table(items: List[ModelInfoData], top_n: int = 10) -> None:
        ...
        logger.info("\n" + "=" * 120)
        logger.info(
            f"📊 分析结果汇总（共 {len(items)} 项，显示前 {len(display_items)} 项）"
        )
        logger.info("=" * 120)
        ...
        for i, item in enumerate(display_items, 1):
            ...
            if is_run_summary:
                ...
                # 路径过长时截断
                path_display = (
                    item.path[:28] + "..." if len(item.path) > 30 else item.path
                )
...
class ModelAnalysisService:
    ...
    def summarize_runs(
        ...
        """汇总多个训练任务的结果"""
        logger.info(
            f"📊 开始汇总训练结果 (模式: {run_dir_pattern}, 根目录: {root_dir})"
        )

        # 查找匹配目录
        run_dirs = PathScanner.find_run_directories(
            pattern=run_dir_pattern, root_dir=root_dir
        )
        if not run_dirs:
    ...
    def compare_models_by_dir(
        ...
        # 1. 查找符合模式的目录
        target_dirs = PathScanner.find_run_directories(
            pattern=dir_pattern, root_dir=root_dir
        )
        if not target_dirs:
        ...
        for dir_path in target_dirs:
            pth_files = PathScanner.find_model_files(
                pattern=model_file_pattern, root_dir=dir_path
            )
            if pth_files:
                ...
                logger.info(
                    f"  ├─ 目录 {os.path.basename(dir_path)}: 未找到模型文件，跳过"
                )
    ...
    def compare_latest_models(
        ...
        # 1. 查找符合模式的目录（按修改时间倒序）
        matched_dirs = PathScanner.find_run_directories(
            pattern=pattern, root_dir=root_dir
        )
        if not matched_dirs:
        ...
        sorted_dirs = sorted(
            matched_dirs, key=lambda x: os.path.getmtime(x), reverse=True
        )
        ...
        for dir_path in latest_dirs:
            best_models = PathScanner.find_model_files(
                pattern="best_model*.pth", root_dir=dir_path
            )
            if best_models:
                # 取目录中最新修改的最佳模型
                ...
                logger.info(
                    f"⚠️ 目录 {os.path.basename(dir_path)}: 未找到best_model*.pth，跳过"
                )
    ...
    def visualize_training_metrics(run_dir=None, metrics_path=None, root_dir="."):
        ...
        if metrics_path:
            ...
            logger.info(
                f"  ├─ 从指定目录加载指标文件: {run_dir} (找到 {len(target_files)} 个)"
            )
        else:
            # 自动查找最新训练目录
            logger.info("  ├─ 未指定目录或文件，自动查找最新训练目录...")
            latest_dir = PathScanner.get_latest_run_directory(
                pattern="run_*", root_dir=root_dir
            )
            ...
            target_files = PathScanner.find_metric_files(
                pattern="*.json", root_dir=latest_dir
            )
            logger.info(
                f"  └─ 自动选择最新目录: {latest_dir} (找到 {len(target_files)} 个指标文件)"
            )
        ...
        for data in metric_datas:
            logger.info(
                f"  ├─ 可视化指标: {data.metric_type} (来源: {os.path.basename(data.source_path)})"
            )
